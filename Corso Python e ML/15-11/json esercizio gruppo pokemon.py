...
def ottieni_pokemon_da_api(numero):
    ...
    if response.status_code == 200:
        ...
        pokemon = {
            ...
            "peso": data["weight"] / 10,  # Peso in kg
            "altezza": data["height"] / 10  # Altezza in m
        }
        return pokemon
    else:
...
def sfida_pokemon(pokemon1, pokemon2):
    ...
    for turno in range(1, 4):
        ...
        print(f"{pokemon1['nome']} infligge {danno1} danni a {pokemon2['nome']}. HP di {pokemon2['nome']}: {max(hp2, 0)}")
        print(f"{pokemon2['nome']} infligge {danno2} danni a {pokemon1['nome']}. HP di {pokemon1['nome']}: {max(hp1, 0)}")
    ...
    if hp1 > 0 and hp2 > 0:
        if pokemon1['xp'] > pokemon2['xp']:
            print(f"{pokemon1['nome']} vince la sfida!")
        else:
            print(f"{pokemon2['nome']} vince la sfida!")
            return True
    elif hp1 > 0:
        print(f"{pokemon1['nome']} vince la sfida!")
    elif hp2 > 0:
...
def cattura_pokemon():
    ...
    while True:
        ...
        if scelta == "1":
            ...
            if len(pokedex) < 2:
                print("Hai bisogno di almeno due Pokémon nel Pokédex per simulare una sfida!")
            else:
                pokemon_ids = random.sample(list(pokedex.keys()), 2)
                pokemon1 = pokedex[pokemon_ids[0]]
                pokemon2 = ottieni_pokemon_da_api(random.randint(1, 1025))

                if pokemon2 and sfida_pokemon(pokemon1, pokemon2):
                    aggiungi_al_pokedex(pokedex, pokemon2)
                    print(f"{pokemon2['nome']} è stato aggiunto al tuo Pokédex come premio della sfida!")
        elif scelta == "4":
